...
disc_layers.append(nn.weight_norm(dnn.Conv2DDNNLayer(disc_layers[-1], 128, (3,3), pad=1, W=Normal(0.05), nonlinearity=nn.lrelu, name='dcnn1')))
disc_layers.append(nn.weight_norm(dnn.Conv2DDNNLayer(disc_layers[-1], 128, (3,3), pad=1, W=Normal(0.05), nonlinearity=nn.lrelu, name='dcnn2')))
disc_layers.append(nn.weight_norm(dnn.Conv2DDNNLayer(disc_layers[-1], 128, (3,3), pad=1, stride=2, W=Normal(0.05), nonlinearity=nn.lrelu, name='dcnn3')))
disc_layers.append(ll.DropoutLayer(disc_layers[-1], p=0.5))
disc_layers.append(nn.weight_norm(dnn.Conv2DDNNLayer(disc_layers[-1], 256, (3,3), pad=1, W=Normal(0.05), nonlinearity=nn.lrelu, name='dcnn4')))
disc_layers.append(nn.weight_norm(dnn.Conv2DDNNLayer(disc_layers[-1], 256, (3,3), pad=1, W=Normal(0.05), nonlinearity=nn.lrelu, name='dcnn5')))
disc_layers.append(nn.weight_norm(dnn.Conv2DDNNLayer(disc_layers[-1], 256, (3,3), pad=1, stride=2, W=Normal(0.05), nonlinearity=nn.lrelu, name='dcnn6')))
disc_layers.append(ll.DropoutLayer(disc_layers[-1], p=0.5))
disc_layers.append(nn.weight_norm(dnn.Conv2DDNNLayer(disc_layers[-1], 512, (3,3), pad=0, W=Normal(0.05), nonlinearity=nn.lrelu, name='dcnn7')))
disc_layers.append(nn.weight_norm(ll.NINLayer(disc_layers[-1], num_units=256, W=Normal(0.05), nonlinearity=nn.lrelu, name='dnin1')))
disc_layers.append(nn.weight_norm(ll.NINLayer(disc_layers[-1], num_units=128, W=Normal(0.05), nonlinearity=nn.lrelu, name='dnin2')))
...
output_before_softmax_lab = ll.get_output(disc_layers[-1], {x_input: x_lab}, deterministic=False)
output_before_softmax_unl = ll.get_output(disc_layers[-1], {x_input: x_unl}, deterministic=False)
...
x_target_lab = T.matrix()
x_target_unl = T.matrix()
output_lab = T.nnet.softmax(output_before_softmax_lab)
output_unl = T.nnet.softmax(output_before_softmax_unl)
...
# test error
output_before_softmax = ll.get_output(disc_layers[-1], {x_input: x_lab}, deterministic=True)
...
train_batch_disc = th.function(inputs=[x_lab,x_target_lab,labels,x_unl,x_target_unl,lr,unsup_weight_var], outputs=[loss_lab, loss_unl, train_err, output_lab, output_unl], updates=disc_param_updates+disc_avg_updates)
...
ensemble_prediction_lab = np.zeros((len(txs), 10), dtype='float32')
training_targets_lab = np.zeros((len(txs), 10), dtype='float32')
ensemble_prediction_unl = np.zeros((len(trainx_unl), 10), dtype='float32')
training_targets_unl = np.zeros((len(trainx_unl), 10), dtype='float32')
...
for epoch in range(900):
    ...
    epoch_prediction_lab = np.zeros((len(txs), 10), dtype='float32')
    epoch_prediction_unl = np.zeros((len(trainx_unl), 10), dtype='float32')
    ...
    for t in range(nr_batches_train):
        ...
        ll, lu, te, pred_lab, pred_unl = train_batch_disc(trainxa[ran_from:ran_to],
                                      training_targets_lab[lab_inds[ran_from:ran_to]],
                                      trainy[ran_from:ran_to],
                                      trainx_unl[unl_inds[ran_from:ran_to]],
                                      training_targets_unl[unl_inds[ran_from:ran_to]],
                                      lr, unsup_weight)
        loss_lab += ll
        loss_unl += lu
        train_err += te

        epoch_prediction_lab[lab_inds[ran_from:ran_to]] = pred_lab
        epoch_prediction_unl[unl_inds[ran_from:ran_to]] = pred_unl
